...
N=2000;                                     %时刻总量
counter=1:2000
W=randn(1,N)*0.5/2000;               
%过程噪声
X=stepfun(1:N,N/2)*10/2000+W;                      %角速度曲线，单位：度，在k>1000之后以90°每秒的角速度旋转
H=1;                                 %转换矩阵
V=randn(1,N)/2000;                            %测量噪声，因为不够大，噪声不够明显，所以乘以20倍
for k=1:N
...
plot(counter,X,'b',counter,Y,'c',counter,AF,'k');
axis([0 2000 -5 15]);
xlabel('time')
ylabel('°/s')
